...
### 1.4 System Architecture & Deployment

The system runs as a **lightweight localhost web application** with a Python backend. The developer interacts with the negotiation through a browser UI. Everything downstream (test generation, evaluation, Jira updates) is orchestrated by the backend.
...
│                                                          │
│  Single HTML page (Alpine.js + Prism.js)                 │
│  ├── View 1: Negotiation (split-pane chat + spec preview)│
│  ├── View 2: Execution (pipeline log + verdicts)         │
│  └── View 3: Results (evidence breakdown + Jira link)    │
│                                                          │
│  Communicates via 5 REST endpoints (see Section 9)       │
└──────────────┬───────────────────────────────────────────┘
               │ HTTP (localhost)
               ▼
┌──────────────────────────────────────────────────────────┐
...
│  localhost:8000                                           │
│                                                           │
│  ├── Negotiation harness (state machine, context object)  │
│  ├── AI client (calls internal Claude/Sherpa endpoint)    │
│  ├── Jira client (reads/writes Jira Cloud tickets)        │
│  ├── Spec compiler (context → YAML)                       │
│  ├── Skill dispatcher (spec routing → skill agents)       │
│  ├── Test runner (subprocess: gradle/npm/pytest)          │
│  ├── Evaluator (results → verdicts per AC checkbox)       │
│  └── Serves the single HTML page via StaticFiles          │
└──────────┬───────────────────────────┬───────────────────┘
           │                           │
           ▼                           ▼
┌────────────────────┐   ┌─────────────────────────────────┐
│  EXTERNAL: AI       │   │  EXTERNAL: Jira Cloud (Free)    │
│                     │   │                                 │
│  Internal Claude/   │   │  REST API v3                    │
│  Sherpa endpoint    │   │  ├── Read ticket + AC           │
│                     │   │  ├── Write checkbox updates      │
│  Called during      │   │  ├── Post evidence comments      │
│  negotiation only   │   │  └── Transition ticket           │
│  (Phases 1-7)       │   │                                 │
│                     │   │  Called during execution only    │
│  Zero AI calls      │   │  (post-evaluation)              │
│  during execution   │   │                                 │
└─────────────────────┘   └─────────────────────────────────┘
```

#### Why Localhost Web, Not Terminal

- **Demo impact**: A split-pane browser UI with conversation bubbles, a live YAML preview, and animated checkmarks is immediately legible to judges. Terminal output, no matter how polished, reads as "developer tool" rather than "product."
- **Parallel development**: The UI person builds the entire frontend against mock API responses while the backend team builds the real negotiation. They integrate through a 5-endpoint REST contract.
- **Richer interaction**: Clickable approve/edit/skip buttons, collapsible negotiation phases, syntax-highlighted YAML with diff markers, phase progress bars, and a streaming execution dashboard.
- **Presentation-ready**: Full-screen browser during the 5-minute demo. Judges can read everything without squinting at monospace terminal text.

#### The File System as State Store

The system uses the **file system as its state store**, not a database. The spec YAML file is the handoff contract between the negotiation phase and the execution phase. Everything is committable, auditable, and resumable.

```
.verify/
├── constitution.yaml              # Repo context (written by init, once per repo)
├── specs/
│   └── PROJ-1234.yaml             # The spec (written by negotiation, read by execution)
├── logs/
│   └── PROJ-1234-negotiation.md   # Full conversation transcript
├── generated/
│   ├── UserProfileVerificationTest.java  # Generated by skill agent
│   └── user-profile-latency.json         # Generated by skill agent
└── results/
    ├── junit-results.xml          # Test execution output
    └── PROJ-1234-verdicts.json    # Evaluator output
```

This means the pipeline is **resumable**: if the negotiation finishes but test execution fails, you fix the issue and re-run execution — it reads the existing spec and picks up where it left off. If Jira is down, the verdicts are already saved and you can push to Jira later.

#### The User Workflow (PO → Developer → Tool)

The PO's job doesn't change — they write AC in their natural fuzzy language. The tool takes friction off the **developer**, who currently must manually translate fuzziness into tests, PRs, and Jira updates.

```
PO writes fuzzy AC in Jira (their job, unchanged)
    ↓
Developer opens tool: localhost:8000
    ↓
Developer enters Jira ticket key
    ↓
AI negotiation helps the DEVELOPER formalize the AC into a spec
(Essentially automating what should happen during backlog refinement —
the developer asking "what do you mean by 'view their profile'?")
    ↓
Developer approves the spec
    ↓
Everything downstream is automated: 
  tests generated → executed → evaluated → Jira updated
```
...
**Category 4: CLI Exit Code Verification** — Runs a CLI tool and checks whether it returns exit code 0. Many "untestable" things are verified this way — there's no JUnit XML to parse, no tags to match. It's binary: the tool either passes or fails. This covers: dependency vulnerability scans, secret detection, SAST/DAST, OpenAPI diff checks, database migration validation, accessibility scanners, linting.

### 4.4 The Universe of "Usually Untested" Things

When a PO writes an AC, they're often implicitly requiring things that no test framework covers. The negotiation engine (particularly Phase 5 — Invariant Extraction and Phase 6 — Completeness Sweep) should surface these proactively, even when the PO didn't write them. Here is the comprehensive taxonomy:

**Configuration & Infrastructure Correctness** — Things that are "correct" if a config file or infrastructure state matches the spec. No application logic to test — the artifact IS the proof:

| What | Example AC Implication | Verification Type | What Skill Generates |
|------|----------------------|-------------------|---------------------|
| Alerting rules | "Must alert if latency > 500ms" | `deployment_check` | NRQL/PromQL JSON config |
| Dashboard panels | "Team must have visibility into error rates" | `deployment_check` | Grafana/NR dashboard JSON |
| Feature flags | "Feature behind a flag for gradual rollout" | `config_validation` | LaunchDarkly flag config |
| Rate limiting | "Endpoint limited to 100 req/min" | `config_validation` | API gateway config snippet |
| CORS configuration | "Only accept requests from app.example.com" | `test_result` | Test hitting endpoint with wrong origin |
| Container security | "Service must not run as root" | `config_validation` | Dockerfile/K8s securityContext check |
| Resource limits | "Must have memory limits to prevent OOM" | `config_validation` | K8s deployment resource limits check |
| Environment config | "Must connect to correct database" | `config_validation` | Deployment config env var check |

**Observability & Operational Readiness** — Can the on-call team detect and respond to problems:

| What | Example AC Implication | Verification Type | What Skill Generates |
|------|----------------------|-------------------|---------------------|
| Structured logging | "All API calls logged with request ID, user ID" | `test_result` | Test asserting log output fields |
| Distributed tracing | "Requests traceable across services" | `config_validation` | OTel config with span definitions |
| Health endpoint | "Service exposes /health for load balancer" | `api_health_check` | HTTP check against /health |
| Readiness probes | "K8s knows when service is ready" | `config_validation` | K8s deployment probe definitions |
| Runbook exists | "On-call knows how to respond to alerts" | `deployment_check` | Runbook markdown at expected path |
| Graceful degradation | "Return cached data if downstream unavailable" | `config_validation` | Circuit breaker config check |

**Security Posture** — Not just "does auth work" but "is the security architecture correct":

| What | Example AC Implication | Verification Type | What Skill Generates |
|------|----------------------|-------------------|---------------------|
| Dependency CVEs | "No known critical vulnerabilities" | `cli_exit_code` | `npm audit --audit-level=critical` |
| Static analysis | "No SQL injection or XSS" | `cli_exit_code` | SonarQube/Semgrep scan command |
| Security headers | "X-Content-Type-Options, HSTS present" | `test_result` | Test checking response headers |
| Secrets in code | "No API keys committed to repo" | `cli_exit_code` | `gitleaks detect --source .` |
| Auth enforcement | "No endpoint accidentally public" | `test_result` | Test that all endpoints require auth |
| Audit trail | "All mutations logged for compliance" | `test_result` | Test asserting audit log entries |

**Data Governance & Compliance** — Regulatory/policy requirements usually verified by humans:

| What | Example AC Implication | Verification Type | What Skill Generates |
|------|----------------------|-------------------|---------------------|
| Data classification | "PII fields must be tagged" | `test_result` | Test checking X-Data-Classification header |
| Data retention | "Delete data after 30 days of closure" | `config_validation` | TTL policy or scheduled job config |
| Accessibility (WCAG) | "UI meets WCAG 2.1 AA standards" | `cli_exit_code` | `pa11y --standard WCAG2AA` |
| Localization | "UI supports Spanish and French" | `config_validation` | Translation file key completeness check |
| Consent management | "Users must consent before marketing" | `test_result` | Test that consent check exists in code path |

**Documentation & Knowledge** — Whether humans can understand and evolve the system:

| What | Example AC Implication | Verification Type | What Skill Generates |
|------|----------------------|-------------------|---------------------|
| API documentation | "Endpoint documented in OpenAPI spec" | `cli_exit_code` | `openapi-diff` spec vs implementation |
| Changelog | "Change documented in CHANGELOG.md" | `deployment_check` | Changelog entry existence check |
| ADR exists | "Design decision recorded" | `deployment_check` | ADR file existence at expected path |

**Deployment & Release Correctness** — The deployment process itself, not the code:

| What | Example AC Implication | Verification Type | What Skill Generates |
|------|----------------------|-------------------|---------------------|
| DB migration | "Schema change is backwards-compatible" | `cli_exit_code` | `flyway validate` command |
| Backwards compat | "Existing clients must not break" | `test_result` | Contract tests against previous API version |
| Smoke test | "Endpoint returns 200 after deployment" | `api_health_check` | HTTP check against deployed endpoint |
| Feature toggle default | "New features ship disabled" | `config_validation` | Flag default state check |

**Cross-System Contracts** — Agreements between systems nobody owns end-to-end:

| What | Example AC Implication | Verification Type | What Skill Generates |
|------|----------------------|-------------------|---------------------|
| Event schema | "Published event matches consumer schema" | `config_validation` | Schema validation against registry |
| Downstream SLA | "Don't call payment service > 50 req/s" | `config_validation` | Rate limiter/circuit breaker config |
| API version contract | "V1 available for 6 months after V2" | `api_health_check` | HTTP check against V1 endpoint |
| Message ordering | "Events processed in order per user" | `config_validation` | Queue FIFO/partition key config check |

### 4.5 The Hierarchy of Provability

Not all "proofs" are equally strong. The system should be transparent about this — each verification type has a confidence level:

| Strength | Verification Type | What It Actually Proves | Known Limitation |
|----------|------------------|------------------------|------------------|
| **Strongest** | `test_result` (unit/integration) | Code behaves correctly for specific inputs | Only proves tested scenarios, not ALL scenarios |
| **Strong** | `test_result` (property-based) | Code satisfies invariant for random inputs | Probabilistic, not exhaustive |
| **Strong** | `cli_exit_code` (SAST/CVE scan) | No known vulnerabilities at scan time | New CVEs discovered tomorrow aren't covered |
| **Medium** | `deployment_check` (config file) | Config artifact exists and is structurally valid | Doesn't prove the config is actually deployed/active |
| **Medium** | `config_validation` (entry check) | Config contains required entries | Doesn't prove the entries are correct values, just present |
| **Medium** | `api_health_check` (live probe) | Endpoint responds correctly right now | Doesn't prove it will respond correctly in 5 minutes |
| **Weaker** | `cli_exit_code` (a11y scanner) | No violations found by the scanner | Scanners don't catch all violations |
| **Weakest** | `deployment_check` (doc exists) | Documentation exists at expected path | Doesn't prove documentation is accurate or complete |

Each verification entry in the spec can include a `confidence` field for transparency:

```yaml
- ref: "REQ-001.SEC-001"
  verification_type: cli_exit_code
  confidence: strong
  description: "No critical dependency vulnerabilities"
  caveat: "Only covers known vulnerabilities at scan time"
```

This is intellectually honest and makes the demo *stronger* — you're showing systematic verification across multiple dimensions with transparent confidence levels, not claiming the system proves everything perfectly.

### 4.6 The Tag Contract: How Tests Link Back to Spec Refs
...
### 4.7 Traceability Map: Worked Example
...
@strategy("cli_exit_code")
def eval_cli_exit_code(ref, spec, test_results, check_details):
    """
    Run a CLI command and check the exit code.
    Used for: secret scanners (gitleaks), SAST (semgrep, sonarqube),
    dependency audits (npm audit, snyk), OpenAPI diff, DB migrations 
    (flyway validate), accessibility scanners (pa11y), linting.
    
    This handles the large category of "untestable" things that are 
    verified by running a tool and checking pass/fail.
    """
    import subprocess
    
    command = check_details.get("command", "")
    expected_exit_code = check_details.get("expected_exit_code", 0)
    timeout_seconds = check_details.get("timeout_seconds", 120)
    
    if not command:
        return {"passed": False, "details": "No command specified in check_details"}
    ...
    try:
        ...
        output_preview = result.stdout[:500] if result.stdout else result.stderr[:500]
        ...
        return {
            ...
            "details": (
                ...
                f"Output: {output_preview if not passed else 'clean'}"
            )
        }
    except subprocess.TimeoutExpired:
        return {
            "passed": False, 
            "details": f"Command timed out after {timeout_seconds}s: {command}"
        }
    except Exception as e:
        return {"passed": False, "details": f"Command execution failed: {e}"}
...
| **Verification Skills** | | |
| JUnit/Jest skill (test gen) | MVP | Generate one test file with tagged spec refs |
...
| **Evaluation & Jira** | | |
| Test result parser (JUnit XML) | MVP | Parse test results, match tags to spec refs |
| Deployment check evaluator | STRETCH | Validate generated config files exist/are valid |
| CLI exit code evaluator | STRETCH | Run `npm audit` / `gitleaks` and check exit code |
...
| **Web UI** | | |
| Negotiation view (split-pane) | MVP | Chat left, spec preview right, phase progress top |
| Execution view (pipeline log) | MVP | Streaming status of generation + execution + Jira |
| Results view (evidence table) | STRETCH | Final verdict breakdown with Jira link |
...
## 9. Web UI & API Contract

### 9.1 Tech Stack

The UI is a **single-page application** served by the Python backend. No React, no build step, no npm for the frontend.

| Component | Technology | Rationale |
|-----------|-----------|-----------|
| Backend | **FastAPI** (Python) | Async, auto-docs, SSE support, serves static files |
| Frontend | **HTML + Alpine.js + Prism.js** | Alpine.js adds reactivity via HTML attributes (8kb, zero build). Prism.js for YAML syntax highlighting. |
| AI Client | **Anthropic SDK** or internal Sherpa client | Structured output for spec generation |
| Jira Client | **requests** library | Simple REST calls to Jira Cloud API v3 |
| Streaming | **Server-Sent Events (SSE)** | 5 lines of JS on client, 10 lines of Python on server — way simpler than WebSocket |
| State | **File system** | Spec YAML, results JSON — no database |

The entire frontend is three files: `static/index.html`, `static/style.css`, `static/app.js`. Total ~300-500 lines.

### 9.2 API Endpoints

Lock this contract in immediately — the UI person and backend team build in parallel against it.

```
POST /api/start
  Body: { "jira_key": "PROJ-1234" }
  Response: { 
    "session_id": "abc123",
    "jira_summary": "User profile endpoint",
    "acceptance_criteria": [
      { "index": 0, "text": "User can view their profile", "checked": false },
      { "index": 1, "text": "Profile responds within 500ms", "checked": false },
      { "index": 2, "text": "Data never exposed to others", "checked": false }
    ],
    "current_phase": { "number": 1, "name": "Interface Discovery", "total": 7 }
  }

POST /api/message
  Body: { "session_id": "abc123", "content": "Yes, GET /api/v1/users/me is correct" }
  Response: {
    "ai_message": "Great. Moving to Phase 2...",
    "current_phase": { "number": 2, "name": "Happy Path Contract", "total": 7 },
    "spec_preview": "requirements:\n  - id: REQ-001\n    ...",
    "questions": [
      "What fields constitute 'profile data'?",
      "Is avatar nullable?"
    ]
  }
...
POST /api/approve
  ...
  Response: {
    "spec_path": ".verify/specs/PROJ-1234.yaml",
    "spec_content": "... full YAML ...",
    "routing_summary": [
      { "skill": "junit5_controller_test", "ref_count": 10, "output": "...Test.java" },
      { "skill": "newrelic_alert_config", "ref_count": 1, "output": "...latency.json" }
    ]
  }
...
POST /api/execute
  ...
  Events:
    { "stage": "generating", "skill": "junit5_controller_test", "status": "complete" }
    { "stage": "generating", "skill": "newrelic_alert_config", "status": "complete" }
    { "stage": "running_tests", "progress": "8/12" }
    { "stage": "running_tests", "progress": "12/12", "status": "all_passed" }
    { "stage": "evaluating", "ac_checkbox": 0, "passed": true, "summary": "12/12" }
    { "stage": "evaluating", "ac_checkbox": 1, "passed": true, "summary": "2/2" }
    { "stage": "evaluating", "ac_checkbox": 2, "passed": true, "summary": "5/5" }
    { "stage": "updating_jira", "action": "checkbox_0_ticked" }
    { "stage": "updating_jira", "action": "checkbox_1_ticked" }
    { "stage": "updating_jira", "action": "checkbox_2_ticked" }
    { "stage": "updating_jira", "action": "evidence_posted" }
    { "stage": "updating_jira", "action": "transitioned_to_done" }
    { "stage": "complete", "all_passed": true }

GET /api/spec/{session_id}
  Response: { "spec_content": "... current YAML state ...", "status": "draft|approved" }
```

### 9.3 UI Views

**View 1: Negotiation** — Split-pane with chat conversation (left) and live YAML spec preview (right). Phase progress bar at top. Input box at bottom with Send and Approve buttons.

**View 2: Execution** — Pipeline log streaming on left (generating → running → evaluating → updating Jira), spec with verification entries lighting up green on right. Each step appears in real-time via SSE.

**View 3: Results** — Full evidence breakdown grouped by AC checkbox. Each ref shows verification type, pass/fail, and details. Link to open the Jira ticket.

### 9.4 Mock API for Parallel Development

The UI person creates a `mock_api.py` that returns hardcoded responses for all 5 endpoints. This lets them build and polish the entire frontend while the backend team builds the real negotiation engine. Integration is swapping the import:

```python
# During UI development:
from mock_api import router as api_router
...
## 10. Team Mapping & Parallel Workstreams

### 10.1 Team Assignment (5 People)

| Person | Role | What They Build | Integration Contract |
|--------|------|----------------|---------------------|
| **Person 1 (Maggie)** | Plumber | Jira client (read tickets, write checkboxes, post comments, transition status). Backend endpoints: `/api/start` (Jira fetch), Jira update logic in `/api/execute`. | Consumes: verdicts JSON. Produces: Jira API calls. |
| **Person 2 (Danny)** | Negotiation Engine | Harness, AI client, Phase 1-4 skills, spec compiler. Backend endpoints: `/api/message`, `/api/approve`. | Consumes: Jira AC. Produces: spec YAML. |
| **Person 3 (Chris)** | Specs → Tests | Skill framework, primary test generation skill (JUnit or Jest), tag enforcement. Generation logic within `/api/execute`. | Consumes: spec YAML + constitution. Produces: tagged test files. |
| **Person 4 (Mike)** | Evaluation & Pipeline | Test runner, result parsers (JUnit XML, Jest JSON), evaluator, verdict aggregation. Evaluation + Jira wiring within `/api/execute`. | Consumes: test results + spec traceability map. Produces: verdicts JSON. |
| **Person 5 (Joe)** | UI + Demo | HTML/CSS/JS frontend, mock API, demo narrative, dummy app, presentation deck. Serves via FastAPI StaticFiles. | Consumes: 5 API endpoints. Produces: the visual experience. |

### 10.2 Parallel Workstream Diagram

```
Day 1 (Hours 1-4): Build in Parallel
┌──────────────────────────────────────────────────────────────┐
│                                                              │
│  Joe (UI):          Build entire frontend against mock API   │
│  Maggie (Plumber):  Build Jira client against real Jira      │
│  Danny (Negotiate): Build harness + Phase 1-2 skills         │
│  Chris (Skills):    Build skill framework + test generator    │
│  Mike (Evaluate):   Build evaluator + result parsers          │
│                                                              │
└──────────────────────────────────────────────────────────────┘

Day 1 (Hours 5-8): Integrate
┌──────────────────────────────────────────────────────────────┐
│                                                              │
│  Danny + Maggie:  Wire Jira → negotiation → spec emission    │
│  Chris + Mike:    Wire spec → test generation → evaluation    │
│  Mike + Maggie:   Wire evaluation verdicts → Jira updates     │
│  Joe:             Swap mock API for real backend, polish UI   │
│                                                              │
└──────────────────────────────────────────────────────────────┘

Day 2 (Hours 9-12): Polish + Demo Prep
┌──────────────────────────────────────────────────────────────┐
│                                                              │
│  Everyone:  End-to-end run-throughs, fix integration bugs    │
│  Joe:       Final UI polish, build presentation deck          │
│  Danny:     Add Phase 3-4 skills (failure mode enumeration)   │
│  Chris:     Add stretch skills (New Relic, Gherkin)           │
│  Maggie:    Add evidence comment formatting                   │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```

### 10.3 Integration Contracts (Handoff Files)

Each team member can test independently using these file-based contracts:

| Contract | Producer | Consumer | Format |
|----------|----------|----------|--------|
| Raw AC data | Maggie (Jira client) | Danny (negotiation) | JSON: `{ jira_key, summary, acceptance_criteria[] }` |
| Spec YAML | Danny (negotiation) | Chris (skills), Mike (evaluator) | YAML at `.verify/specs/{key}.yaml` |
| Constitution | Hardcoded (everyone) | Danny (negotiation), Chris (skills) | YAML at `.verify/constitution.yaml` |
| Generated test files | Chris (skills) | Mike (evaluator) | Test file at spec's `verification[].output` path |
| Test results | Mike (test runner) | Mike (evaluator) | JUnit XML at `.verify/results/` |
| Verdicts | Mike (evaluator) | Maggie (Jira updater) | JSON: `[{ ac_checkbox, passed, evidence[] }]` |

Anyone can create a mock version of their producer's output to unblock their own work. Mike can test the evaluator against hand-written JUnit XML. Chris can test the skill against the hardcoded spec from Story 0.2. Maggie can test Jira updates against a hand-written verdicts JSON.

---

## 11. CLI Command Structure

Even though the primary interaction is the web UI, a CLI provides headless execution for CI/CD and debugging:

```bash
# Primary: Launch the web UI (opens browser)
verify start                        # Start server at localhost:8000, open browser

# Step-by-step (for development, debugging, and CI/CD):
verify init                         # Scan repo, generate constitution
verify negotiate PROJ-1234          # Interactive negotiation → spec YAML (uses web UI)
verify generate PROJ-1234           # Read spec, dispatch skills, emit verification artifacts
verify run PROJ-1234                # Execute tests, parse results
verify evaluate PROJ-1234           # Evaluate results against spec traceability map
verify update-jira PROJ-1234        # Tick checkboxes, post evidence, transition

# Shortcuts:
verify execute PROJ-1234            # generate + run + evaluate + update-jira
verify PROJ-1234                    # negotiate + execute (the full pipeline)

# Utilities:
verify status PROJ-1234             # Show current state of the pipeline for this ticket
verify show-spec PROJ-1234          # Pretty-print the spec YAML
verify show-verdicts PROJ-1234      # Show the evaluation results
verify show-evidence PROJ-1234      # Show what would be posted to Jira
```

For the hackathon, only `verify start` (web UI mode) needs to work. The step-by-step commands are stretch goals for production readiness.

---

## 12. Development Setup

### 12.1 Jira Cloud (Free Plan)

Jira Cloud's free plan is free forever for up to 10 users. It includes full REST API access, Scrum/Kanban boards, and the ability to create tickets with AC checkboxes — everything the hackathon needs.

**Setup steps (5 minutes):**
1. Go to `atlassian.com/software/jira/free` and sign up with a personal email
2. Create a site (e.g., `yourname-hackathon.atlassian.net`)
3. Create a project (e.g., key: `DEMO`)
4. Create a ticket with 3 AC checkboxes in the description
5. Generate an API token at `id.atlassian.com/manage-profile/security/api-tokens`

**API authentication:**
```python
import requests

JIRA_BASE = "https://yourname-hackathon.atlassian.net"
AUTH = ("your-email@gmail.com", "your-api-token")  # Basic auth

# Test: fetch a ticket
response = requests.get(f"{JIRA_BASE}/rest/api/3/issue/DEMO-1", auth=AUTH)
print(response.json()["fields"]["summary"])
```

**AC checkbox format in Jira Cloud:** Jira Cloud uses Atlassian Document Format (ADF). Checkboxes are `taskList` / `taskItem` nodes in the description JSON. To tick a checkbox, change `"state": "TODO"` to `"state": "DONE"` and PUT the updated description back. This is fiddly but fully supported on the free plan.

### 12.2 Python Environment

```bash
# Create project
mkdir verify-engine && cd verify-engine
python -m venv .venv && source .venv/bin/activate

# Core dependencies
pip install fastapi uvicorn pyyaml requests

# AI client (pick one based on internal endpoint)
pip install anthropic  # If using Claude directly

# Dev utilities
pip install rich       # Pretty console output for debugging
pip install httpx      # Async HTTP client (optional, for FastAPI)
...
JIRA_BASE_URL=https://yourname-hackathon.atlassian.net
JIRA_EMAIL=your-email@gmail.com
JIRA_API_TOKEN=your-api-token
AI_API_KEY=your-claude-or-sherpa-key
AI_ENDPOINT=https://internal-ai-endpoint.example.com  # If using internal
```

### 12.4 Day-One Checklist

Before anyone writes a line of code, these should be done:

- [ ] Jira Cloud free instance created and accessible
- [ ] Demo ticket created with 3 AC checkboxes
- [ ] API token generated and tested (can fetch the demo ticket via curl)
- [ ] Python venv set up with FastAPI + requests
- [ ] AI endpoint accessible (Claude API key or internal Sherpa token)
- [ ] Git repo created with `.verify/` directory structure
- [ ] Hardcoded constitution YAML written for the dummy app
- [ ] Hardcoded spec YAML written for the demo ticket (Story 0.2)
- [ ] API contract (Section 9.2) agreed upon by all team members
- [ ] Dummy app created and running locally (Story 0.1)
