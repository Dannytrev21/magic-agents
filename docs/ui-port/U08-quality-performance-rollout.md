...
- A 2026-04-02 gap-detection follow-up found no unfinished UI-port stories, added a page-level integration assertion that the final negotiation approval auto-switches into the verification workspace, and tightened the mocked Playwright happy path to wait for the phase approval control to become actionable before the terminal click. `npm run test:ui` and `npm run test:e2e:chromium` both passed after the fix.
