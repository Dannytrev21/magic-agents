...
- The browser suite now depends on a local Playwright browser install and on running from the active local worktree. A 2026-04-01 rerun passed once browsers were installed into the same `ui/.playwright/browser-cache` used by the suite.
