...
- Run the browser install and the e2e suite from the same local worktree. A 2026-04-01 re-run passed once Playwright browsers were installed into the active `ui/.playwright/browser-cache` and the suite was executed from an unrestricted local worktree under `/Users/...`.
