...
- Real-time event handling should use the `EventStoreProvider` + typed selector hooks (`usePhaseEvents`, `useBudgetEvents`, etc.) from `eventStore.ts` rather than ad-hoc component state. `AppProviders` owns the shared store mount, and `usePhaseWorkspaceModel()` is the preferred shell-level bridge from `useSSE()` into the workspace chrome.
...
- Run `npm run lint` in [`/Users/dannytrevino/development/magic-agents/ui`](/Users/dannytrevino/development/magic-agents/ui) whenever you touch frontend source, Playwright specs, or frontend tooling config. The lint config intentionally ignores generated Playwright/browser output and build artifacts so only tracked UI code participates in the typed lint gate.
...
- Use `npm run test:ci` in [`/Users/dannytrevino/development/magic-agents/ui`](/Users/dannytrevino/development/magic-agents/ui) when you want the full non-browser frontend gate in one command. It now runs lint, Vitest, the production build, and bundle budgets in sequence.
- Vitest is intentionally serialized in [`/Users/dannytrevino/development/magic-agents/ui/vitest.config.ts`](/Users/dannytrevino/development/magic-agents/ui/vitest.config.ts) with `maxWorkers: 1` and `fileParallelism: false` because the jsdom-heavy UI suite can starve and trip false timeouts under worker contention even when the same files pass in isolation.
...
- Run `npm run test:e2e:chromium` after changing intake fallback copy, phase-approval affordances, or other operator-journey wording that may stay green in Vitest but fail in the rendered browser flow.
- Keep shared jsdom/browser-runtime fallbacks in [`/Users/dannytrevino/development/magic-agents/ui/src/test/setup.ts`](/Users/dannytrevino/development/magic-agents/ui/src/test/setup.ts). If a new UI surface needs observer, media-query, or scroll APIs in tests, wire them there instead of re-declaring per-test shims.
- When [`/Users/dannytrevino/development/magic-agents/docs/ui-port/progress.json`](/Users/dannytrevino/development/magic-agents/docs/ui-port/progress.json) already reports the UI-port epic complete, treat a failing `npm run test:ci`, a failing `npm run test:e2e:chromium`, or drift between `progress.json` and the epic markdown files as the next actionable UI gap before adding new UI-port scope.
- Treat `ui/config/bundle-budgets.json` as a product constraint, not a release valve. When `npm run test:budgets` fails, reduce shipped shell bytes or split heavy surfaces before raising thresholds unless the feature scope itself changed.
...
- Set `PW_SKIP_WEBSERVER=1` when the app under test is already running, or override the target app URL with `PW_BASE_URL=http://host:port`. Failed runs retain screenshots under the ignored Playwright output folders in the UI workspace. Set `PW_TRACE=1` when you also want Playwright trace archives and the host can persist them reliably.
