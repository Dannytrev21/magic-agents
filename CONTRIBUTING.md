...
- Install the local Playwright browsers once with `npm run test:e2e:install` and run `npm run test:e2e` for deterministic mock-mode browser coverage. Use `npm run test:e2e:chromium`, `npm run test:e2e:firefox`, or `npm run test:e2e:webkit` when you need to isolate host-specific browser failures. Failed runs retain traces/screenshots under the ignored Playwright output folders in the UI workspace.
