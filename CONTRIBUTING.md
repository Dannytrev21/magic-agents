...
- Install the local Playwright browser once with `npm run test:e2e:install` and run `npm run test:e2e` for deterministic mock-mode browser coverage. Failed runs retain traces/screenshots under the ignored Playwright output folders in the UI workspace.
- In restricted macOS sandboxes, Playwright browsers can still abort before the first page load even after install. The current automation environment reproduces a Chromium Mach bootstrap permission failure (`bootstrap_check_in ... Permission denied (1100)`), so rerun browser coverage on a machine that permits headless browser launch.
