...
- The shell-level bridge in [`/Users/dannytrevino/development/magic-agents/ui/src/features/workspace/phaseWorkspaceModel.ts`](/Users/dannytrevino/development/magic-agents/ui/src/features/workspace/phaseWorkspaceModel.ts) now dispatches every negotiation SSE through `useSSE(..., { onEvent })` instead of deriving store writes from one `lastEvent` snapshot, which preserves `phase_start` and immediate `phase_progress` pairs when React batches them into the same render and keeps the top-bar stream pill plus phase-progress strip synchronized from one session-scoped source.
...
- The current non-browser shell CSS budget remains 33.5 kB raw and 7 kB gzip. The latest 2026-04-02 verification run measured the shipped shell at 29.95 kB raw and 5.64 kB gzip, so the live stream-status and phase-progress shell chrome remain under budget without relaxing the compressed ceiling.
