...
- On 2026-04-02, the live workspace wiring moved from isolated primitives into the shipped shell: `usePhaseWorkspaceModel` now owns the session-scoped SSE bridge into the shared `EventStoreProvider`, dispatches every negotiation SSE through `useSSE(..., { onEvent })` instead of deriving store writes from one `lastEvent` snapshot, maps `connecting`/`connected`/`reconnecting`/`disconnected` into quiet top-bar copy, and renders phase progress in both the sticky mini-rail and the center workspace header.
...
- The current non-browser shell CSS budget remains 33.5 kB raw and 7 kB gzip. The latest 2026-04-02 verification run measured the shipped shell at 31.43 kB raw and 5.82 kB gzip, so the live stream-status and phase-progress shell chrome remain under budget without relaxing the compressed ceiling.
